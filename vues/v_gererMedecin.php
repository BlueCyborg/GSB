<script>
    var numb = <?php sizeof($specialitesMedecin) ?>; //initilisation du nombre de spécialités en fonction du nombre de médecins
    ...
    function addList() { //Fonction permettant de rajouter une nouvelle spécialitée
        ...
        <?php
        $i = 1;
        // foreach ($specialitesMedecin as $uneSpecialiteMedecin) { ?>
        //     const option<?= $i  ?> = document.createElement("option");
        //     option<?= $i ?>.value = "<?= htmlspecialchars($uneSpecialiteMedecin['SPE_CODE']) ?>";
        //     option<?= $i ?>.text = "<?= htmlspecialchars($uneSpecialiteMedecin['SPE_LIBELLE']) ?>";
        ...
        <form action="index.php?uc=medecin&action=gererMedecin" method="POST" class="d-flex justify-content-center">
            <div class="d-flex flex-column gap-3 justify-content-center align-content-lg-stretch">
                ...
                <div>
                    ...
                    <input class="form-control" type="text" id="nom_medecin" name="nom_medecin" value="<?= htmlspecialchars($medecin['PRA_NOM']) ?>" />
                </div>
                <div>
                    <label for="prenom_medecin" class="form-label">Prénom :</label>
                    <input class="form-control" type="text" id="prenom_medecin" name="prenom_medecin" value="<?= htmlspecialchars($medecin['PRA_PRENOM']) ?>" />
                </div>
                <div>
                    <label for="adresse_medecin" class="form-label">Adresse :</label>
                    <input class="form-control" type="text" id="adresse_medecin" name="adresse_medecin" value="<?= htmlspecialchars($medecin['PRA_ADRESSE']) ?>" />
                </div>
                <div>
                    <label for="cp_medecin" class="form-label">Code postal :</label>
                    <input class="form-control" type="text" id="cp_medecin" name="cp_medecin" value="<?= htmlspecialchars($medecin['PRA_CP']) ?>" />
                </div>
                <div>
                    <label for="ville_medecin" class="form-label">Ville :</label>
                    <input class="form-control" type="text" id="ville_medecin" name="ville_medecin" value="<?= htmlspecialchars($medecin['PRA_VILLE']) ?>" />
                </div>
                <div>
                ...
                <button type="submit" name="submit" class="btn btn-outline-primary">Valider les modifications</button>
            </div>
        </form>
    </div>
</div>